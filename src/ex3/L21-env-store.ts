...
export const applyStore = (store: Store, address: number): Result<Value> =>
  address >= 0 && address < unbox(store.vals).length
...
export const setStore = (store: Store, address: number, val: Value): void => {
  if (address >= 0 && address < unbox(store.vals).length)
...
export const makeExtEnv = (
  vs: string[],
  addresses: number[],
  env: Env
): ExtEnv => ({ tag: "ExtEnv", vars: vs, addresses: addresses, nextEnv: env });
...
export const applyEnvStore = (env: Env, v: string): Result<Value> =>
  bind(applyEnv(env, v), (addr) => applyStore(theStore, addr));
